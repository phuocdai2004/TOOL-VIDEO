...
# 默认字幕字体。Segoe UI 在 Windows 上完整支持越南语重音；非 Windows 环境下
# resolve_font_path() 会回退到项目内置字体，避免启动失败。
DEFAULT_SUBTITLE_FONT = "Segoe UI"
...
def resolve_font_path(font: str) -> str:
    ...
    3. 已知系统字体名 → 返回系统字体路径（如 Windows Segoe UI）
    4. 已知的非 CJK 字体名 → 回退到 DEFAULT_CHINESE_FONT（兼容旧任务）
    5. 其他系统字体名 → 直接返回
    """
    # 已经是绝对路径，直接返回
